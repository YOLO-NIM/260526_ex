// 상속
class Animal {
  count = 0; // 인스턴스 귀속 : new로 만들면 그 객체에서 count 영역이 생김
  ...
  constructor(name) {
    ...
    // 클래스/인스턴스에 속한 함t수는 메서드 -> static 메서드
  }
  speak() {
...
class Cat extends Animal {
  // 상속: 이미 기존에 구현해 놓은 클래스를 써서 일부 내용을 공유하는 신규 클래스를 만드는 것
  #age;
  constructor(name, age) {
    // 냅두면 상속받은 원래 출처의 생성자가 기본으로 활성화
    // 우리가 직접 생성자를 만들면 그게 새로운 게 된다.
    // 상속 받은 직전 클래스의 생성자 호출 => super(매개변수...)
...
// Static - 인스턴스와 다른 저장 공간이므로 이름이 겹쳐도 된다
// 클래스.(이름). 변수/함수 다 된다
// 쓸 일이 있으면 class.()로 쓰면 된다
