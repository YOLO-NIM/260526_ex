...
try {
  ...
  console.log(e); // MyError: 무언가 잘못됐어 단단ㄷ히
...
function f2() {
  throw new Error("f2에서 발생"); // 최상위에 있는 실행 상황이나 catch를 만날 때까지는 throw -> 블록을 뚫고 나감
  return true;
}
try {
  console.log("f", f());
...
try {
  console.log(1);
  a.a; // 일종의 return, break, countinue처럼 흐름제어 -> 지금처럼
  console.log(2);
} finally {
  // 앞에 에러가 있든 없든 이제 뒤에 이 작업을 하겠다
