...
// 자바스크립트의 기본 정렬은 '문자열'화 시킨 다음에 정렬을 한다
// 13, 2 -> 2가 뒤레 옴
// [1]-3, [2]-.. => 작은 순서에서 큰 순서로 배치하는 것 -> 오름차순
// 오름차순 : 데이터가 등장하는 방향과 데이터를 배치하는 순서/방향이 일치할 때 (ascending order)
// 내림차순 : 데이터가 등장하는 방향과 데이터가 커지는 방향이 반대일 때 (descending order)

console.log(a);
// 자바스크립트에서는 sort 메서드 사용 시 본래값도 변경을 하고, 변경된 값(배열)도 리턴을 한다
// a.toSorted() - 원본에 영향을 미치지 않고 정렬된 결과만 가져다 주는 내장 메서드
function compare(a, b) {
  // 앞 원소, 뒤 원소 2개
  // 1, 5, 13, 8, 2
  // (1,5) -> 15 => + : 1이 더 큰 것, - : 5가 더 큰 것, 0 : 그대로
  return a - b; // b - a : 내림차순
}
...
console.log(a.sort((a, b) => b - a)); // [13, 8, 5, 3, 1]
const a2 = [1, -3, 5, -19, 7];
// 화살표 함수가 가장 일반적으로 고차함수 구현 시 패러미터로 사용됨
console.log(a2.sort((a, b) => Math.abs(a) - Math.abs(b)));

// 배열 내장 메서드 <- 함수를 패러미터로 쓰기 때문에 고차함수
a2.forEach(console.log); // 괄호를 안 한 순수함수가 내장 메서드들을 위한 호출용 함수로 쓸 수 있다.
// () 없이 붙이면 알아서 필요할 때 호출해서 써라 / () 붙으면 -> 값을 써라
a2.forEach((v) => console.log(v));
// 배열 내장 메서드에 패러미터로 들어가는 함수 -> fn(value, inde, array)
...
console.log(a2.map((v, i) => (i % 2 == 0 ? v ** 2 : 0))); // 삼항 연산자
// [ 1, 0, 25, 0, 361 ]
console.log(a2.map((v, i) => (!(i % 2) ? v ** 2 : 0))); // 숏 코딩. (추천하지는 않음)
// 배열 -> for문. 압축되지 않은 표현식 -> 함수화 -> 배열 내장 메서드(고차함수) + 고급 연산자들을 사용해서 표현식 압축
console.log(a2); // map 원본에 영향을 안 미침 -> 복사본을 만든다 [...arr] <- fn으로 하나씩 작업을 한 셈.
...
// 별도의 변수/상수로 새로운 리턴값을 지정해줘야 함(scope, 구현상의 편의 등...)
...
const sum2 = a4.reduce((prev, cur) => {
  // 최초의 직전값(prev)은 0번째 인덱스, -> 1번째 인덕스 (cur) 연산
  console.log("prev", prev, "cur", cur);
  return prev + cur; // 다음 회차의 prev
});
console.log(sum2); // 27
// loadsh - 요새는 많이 안 쓰지만 내장되어 있지 않은 여러 기능들을 담은 라이브러리.
