...
const m = new Map();
m.set("a", 1234); // 같은 값으로 넣었을 때 충돌 문제가 없음
...
// 속도도 느리고 구현 난도도 적지 않은 편 (Set을 직접 다른 자료구조로 구현하기)
...
s.add(12345);
s.add(123456);
// s.has, s.size는 똑같은 역할로 존재
for (const c of s) {
  console.log(c);
}

// set(자료구조로서의) vs set(일반적인 프로그래밍에서의)
// 자료구조 set : 1. 밸류가 곧 키인(중복 제거 연산) 2. 집합 연산을 지원하는 자료구조 (부분집합-mdn 참조)
...
// Java (get) : 없는 것을 호출하면 null
// getOrDefault
// Python([]) : key Error
// Python(get) : None
// setDefault...

// has로 체크를 해서 true, false로 만들거나 비교연산자 등의 테크닉으로 truthy, falsy, undefined를 대응하는 ?? 연산자 등을 활용
